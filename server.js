...
function onMessageHandler (target, context, msg, self) {
  ...
  const channelname = Object.keys(inChat).find(key => key.toLowerCase() === target.replace('#', ''));

  // set the nickname that said the message
  const nickname = context['display-name'];
  ...
  if (commandName === '!blossom') {
    ...
    if ((context.subscriber === false) && (context.mod === false) && (context.badges.broadcaster !== '1')) {
      client.say(target, `@${nickname}: You must be subscribed or added by a mod.`);
      return;
    } else {
      const url = `http://localhost:5002/api/viewtime/${channelname}/${nickname}`;
        ...
        .then(response => {
          if (response.ok) {
            response.json().then((data) => {
              ...
              // console.log(data);
              client.say(target, `@${nickname}, you are in this thing!`);
            });
          } else {
  ...
  if (/^!blossom add /.test(commandName)) {
    ...
    const username = commandName.split(' ')[2];
    ...
    const url = `http://localhost:5002/api/viewtime/${channelname}/${username}`;
        ...
        .then(response => {
          if (response.ok) {
            response.json().then((data) => {
              ...
              client.say(target, `Goodbye @${nickname}, you are out this thing!`);
              // console.log(data);
            });
          } else {
            if (response.status === 404) {
      ...
      .then(response => {
        if (response.ok) {
          response.json().then((data) => {
            ...
            // console.log(data);
            const url = `http://localhost:5002/api/water/${channelname}/${nickname}`;
              ...
              .then(response => {
                if (response.ok) {
                  response.json().then((data) => {
                    ...
                    // console.log(data);
                    client.say(target, `@${nickname}; You watered your flower!`);
                  });
                } else {
                  if (response.status === 404) {
                    console.log('it was a 404');
  ...
  checkIsPlanter(channelname, nickname);
}
...
function checkIsPlanter (channelname, viewername) {
  ...
  const thingerton = inChat[channelname];
  ...
  if (thingerton[viewername] === undefined) {
    ...
    const url = `http://localhost:5002/api/viewtime/${channelname}/${viewername}`;
...
function intervalFunc () {
  ...
  const url = 'http://localhost:5002/api/cycle/update';
...
app.put('/api/is_credited/:channelname/:username/:bool', (req, res) => {
  con.connect(function (err) {
    ...
    con.query(sqlquery, function (err, result, fields) {
      if (err) throw err;
      // console.log(result);
      if (JSON.stringify(result) !== '[]') { res.send(result); } else { res.send(404, 'No results'); }
    });
  });
});
...
app.get('/api/overlay/:channelname', (req, res) => {
  const output = overlayhtml.replace('__REPLACEME__', req.params.channelname);
  // console.log(output)
  res.send(output);
});
...
app.get('/api/viewers/:channelname', (req, res) => {
  con.connect(function (err) {
    ...
    con.query(sqlquery, function (err, result, fields) {
      ...
      if (JSON.stringify(result) !== '[]') { res.send(myresult); } else { res.send(404, 'No results'); }
    });
  });
});
...
app.get('/api/viewtime/:channelname/:username', (req, res) => {
  con.connect(function (err) {
    ...
    con.query(sqlquery, function (err, result, fields) {
      if (err) throw err;
      // console.log(result);
      if (JSON.stringify(result) !== '[]') { res.send(result[0]); } else { res.send(404, 'No results'); }
    });
  });
});
